...
public class Tester {
	public static void main(String[] args) {
		...
		source.subscribe(5000, new OnPriceRefreshedListenner() {

			@Override
			public void onPriceRefreshed(String sourceName, boolean succeed, CoinID from, CoinID to, double rate,
					long timeStamp) {
				System.out.println("sourceName = " + sourceName + "\tsucceed = " + succeed + " " + from.name() + "_"
						+ to.name() + " time = " + TimeUtils.DateToString(timeStamp) + " rate = " + rate);
		...
		gateSource.subscribe(5000, new OnPriceRefreshedListenner() {
